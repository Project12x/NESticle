...
int (__fastcall *m6502MemoryWrite)(WORD, BYTE) = nullptr;
WORD (__fastcall *m6502MemoryRead)(WORD) = nullptr;
...
extern "C" uint8_t read6502(uint16_t address) {
    if (address < 0x2000) {
        ...
        if (m6502ram) return m6502ram[address & 0x7FF];
        return 0;
    }
    if (address >= 0x8000) {
        // ROM access
        if (m6502rom) return m6502rom[address];
        return 0;
    }
    // I/O registers - use callback
    if (m6502MemoryRead) return (uint8_t)m6502MemoryRead(address);
    return 0;
}
...
extern "C" void write6502(uint16_t address, uint8_t value) {
    if (address < 0x2000) {
        ...
        if (m6502ram) m6502ram[address & 0x7FF] = value;
        return;
    }
    if (m6502MemoryWrite) m6502MemoryWrite(address, value);
}
...
DWORD __cdecl m6502GetInfo(void) {
    return 0;
}
